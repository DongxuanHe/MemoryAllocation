...
static inline header * allocate_object(size_t raw_size) {
  ...
  size_t user_rounded = (size_t)((raw_size + 7) & ~7);
  size_t actual_size = ALLOC_HEADER_SIZE + user_rounded;
  if (actual_size < sizeof(header)) {
    actual_size = sizeof(header);
  }

  int index = (user_rounded) / 8 - 1; //index of free list
  int lastList = 0;
  ...
  for (; index < N_LISTS; index++) {
    ...
    if (current != sentinel) {
      lastList = 1;
      break;
    } //in a free list prior the last free list
  }
  ...
  do {/////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    if (possible) {
      header* new_allocation = allocate_chunk(ARENA_SIZE); //allocate a new chunk
      header* prevFencePost = get_header_from_offset(new_allocation, -ALLOC_HEADER_SIZE);
      insert_os_chunk(prevFencePost);
      //check if chunks are together
      if (get_state((header*)((char*)lastFencePost + ALLOC_HEADER_SIZE)) == FENCEPOST) {
        size_t leftSize = lastFencePost->left_size;
        set_state(lastFencePost, UNALLOCATED);
        set_state(prevFencePost, UNALLOCATED);
        header* new_chunk = (header*)((char*)new_allocation - 2 * ALLOC_HEADER_SIZE);
        set_size(new_chunk, get_size(new_allocation) + 2 * ALLOC_HEADER_SIZE);
        header* left = (header*)((char*)new_chunk - leftSize);
        new_chunk->left_size = get_size(left);
        header* last = (header*)((char*)new_chunk + get_size(new_chunk));
        last->left_size = get_size(new_chunk);
        set_state(last, FENCEPOST);
        set_size(last, ALLOC_HEADER_SIZE);
        lastFencePost = last;
        numOsChunks--;
        if (get_state(left) == ALLOCATED) {
          header* sen = &(freelistSentinels[N_LISTS - 1]);
          new_chunk->next = sen->next;
          new_chunk->prev = sen;
          sen->next = new_chunk;
          new_chunk->next->prev = new_chunk;
        }
        else if (get_state(left) == UNALLOCATED) {
          int init_left = (get_size(left) - ALLOC_HEADER_SIZE) / 8 - 1;
          set_size(left, get_size(left) + get_size(new_chunk));
          if (init_left < 58) {
        ...
        lastFencePost = (header*)((char*)new_allocation + get_size(new_allocation));
        header* sent = &(freelistSentinels[N_LISTS - 1]);
        new_allocation->next = sent->next;
        new_allocation->prev = sent;
        sent->next = new_allocation;
        new_allocation->next->prev = new_allocation;
      }
    }
  } while (possible);

  //current: the block initially assigned
  ...
  if (get_size(current) < actual_size + sizeof(header)) {
    ...
    block = (header*)((char*)current + get_size(current) - actual_size);
    set_size(current, get_size(current) - actual_size);
    set_size(block, actual_size);
    ((header*)((char*)block + get_size(block)))->left_size = get_size(block);
    block->left_size = get_size(current);
    set_state(block, ALLOCATED);
...
static inline void deallocate_object(void * p) {
  ...
  header *freeing = ptr_to_header(p);
  header *left = get_left_header(freeing);
  header *right = get_right_header(freeing);
  header *sentinel;
  int init, index; //index of the freelist

  if (get_state(freeing) != ALLOCATED) {
  ...
  if (get_state(left) != UNALLOCATED && get_state(right) != UNALLOCATED) {
    index = (get_size(freeing) - ALLOC_HEADER_SIZE) / 8 - 1;
    if (index > 58) {
      index = 58;
    }
    sentinel = &freelistSentinels[index];
    freeing->next = sentinel->next;
    freeing->prev = sentinel;
    freeing->next->prev = freeing;
    sentinel->next = freeing;
    set_state(freeing, UNALLOCATED);
  }
  else if (get_state(left) == UNALLOCATED && get_state(right) != UNALLOCATED) {
    init = (get_size(left) - ALLOC_HEADER_SIZE) / 8 - 1;
    set_size(left, get_size(left) + get_size(freeing));
    set_state(freeing, UNALLOCATED);
    ...
    if (init != index) {
      left->prev->next = left->next;
      left->next->prev = left->prev;//remove from original list
      sentinel = &freelistSentinels[index];
    ...
    init = (get_size(right) - ALLOC_HEADER_SIZE) / 8 - 1;
    set_size(freeing, get_size(freeing) + get_size(right));
    header* right_to_right = get_right_header(right);
    right_to_right->left_size = get_size(freeing);
    index = (get_size(freeing) - ALLOC_HEADER_SIZE) / 8 - 1;
    if (init > 58) {
      init = 58;
    }
    ...
    if (index == init) {
      ...
      freeing->prev = right->prev;
      freeing->prev->next = freeing;
      freeing->next->prev = freeing;
      right->prev = NULL;
      right->next = NULL;
    }
      ...
      right->prev = NULL;
      right->next = NULL;//remove right in orginal list
      sentinel = &freelistSentinels[index];
      freeing->next = sentinel->next;
      freeing->prev = sentinel;
      sentinel->next = freeing;
    ...
    init = (get_size(left) - ALLOC_HEADER_SIZE) / 8 - 1;
    set_size(left, get_size(left) + get_size(freeing) + get_size(right));
    set_state(freeing, UNALLOCATED);
    header* right_to_right = get_right_header(right);
    right_to_right->left_size = get_size(left);
    index = (get_size(left) - ALLOC_HEADER_SIZE) / 8 - 1;
    //remove right from its freelist in all cases
    right->prev->next = right->next;
    right->next->prev = right->prev;
    right->next = NULL;
    right->prev = NULL;
